...
class MatchGameFAP extends Component {
  state = {
    timer: 60,
  ...
  checkingTheImage = id => {
    ...
    if (id === imagesList[randomNumber].id) {
  ...
  renderingGame = () => {
    const {activeTabId, randomNumber} = this.state
    ...
    return (
      <>
        <img src={randomImage} alt="match" className="image-to-match" />
        <ul className="tab-list-item">
          {tabsList.map(eachTab => (
            <TabItem
              eachTab={eachTab}
              key={eachTab.tabId}
              clickTabToChange={this.clickTabToChange}
        ...
        <ul className="images-list-container">
          {filteredList.map(eachImage => (
            <GameItem
              eachImage={eachImage}
              key={eachImage.id}
              checkingTheImage={this.checkingTheImage}
            />
          ))}
        </ul>
      </>
  ...
  renderGameOver = () => {
    ...
    return (
      <div className="game-over-container">
        ...
        <p className="your-score-number">{score}</p>
        <button
          type="button"
  ...
  render() {
    const {timer, isGameOver, score} = this.state

    return (
      <div className="bg-container">
        <div className="navbar-container">
          <NavBar timerCount={timer} score={score} />
        </div>
        <div className="images-tabs-container">
          {isGameOver ? this.renderGameOver() : this.renderingGame()}
        </div>
      </div>
    )
  }
